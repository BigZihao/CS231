...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        if self.use_batchnorm:
            ...
            self.gamma = [np.ones(dims[i+1]) for i in range(self.num_layers -1)]
            self.beta = [np.zeros(dims[i+1]) for i in range(self.num_layers -1)]
    ...
    def loss(self, X, y=None):
        ...
        if self.use_batchnorm:
            for i in xrange(1, self.num_layers):
                layer[i], cache_layer[i] = affine_norm_relu_forward(layer[i - 1],
                                                                   self.params['W%d' % i],
                                                                   self.params['b%d' % i],
                                                                   self.gamma[i-1],
                                                                   self.beta[i-1],
                ...
                if self.use_dropout:
                    layer[i], cache_dropout[i] = dropout_forward(layer[i], self.dropout_param)
        else:
            for i in xrange(1, self.num_layers):
        ...
        if self.use_batchnorm:
            for i in reversed(xrange(1, self.num_layers)):
                # r = cache_layer[i + 1]
                if self.use_dropout:
                    dx[i + 1] = dropout_backward(dx[i + 1], cache_dropout[i])
                dx[i],  grads['W%d' % i], grads['b%d' % i], dgamma, dbeta = affine_norm_relu_backward(dx[i + 1], cache_layer[i])
